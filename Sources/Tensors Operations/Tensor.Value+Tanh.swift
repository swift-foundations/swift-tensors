...
{
    /// Element-wise hyperbolic tangent (`tanh xᵢ`).
    ///
    /// Routes per-element through `Element._tanh(_:)` from
    /// `Numeric.Transcendental`, which binds the libm `tanh`/`tanhf` family
    /// at the L1/L2 boundary per [PLAT-ARCH-008j].
    @inlinable
    public func tanh() -> Tensor.Value<Element, Rank, Tensor.Layout.Order.Row> {
        self.map { (x: Element) -> Element in Element._tanh(x) }
    }
}
