...
{
    /// Element-wise sine (`sin xᵢ`).
    ///
    /// Routes per-element through `Element._sin(_:)` from
    /// `Numeric.Transcendental`, which binds the libm `sin`/`sinf` family
    /// at the L1/L2 boundary per [PLAT-ARCH-008j].
    @inlinable
    public func sin() -> Tensor.Value<Element, Rank, Tensor.Layout.Order.Row> {
        self.map { (x: Element) -> Element in Element._sin(x) }
    }
}
