...
{
    /// Element-wise hyperbolic sine (`sinh xᵢ`).
    ///
    /// Routes per-element through `Element._sinh(_:)` from
    /// `Numeric.Transcendental`, which binds the libm `sinh`/`sinhf` family
    /// at the L1/L2 boundary per [PLAT-ARCH-008j].
    @inlinable
    public func sinh() -> Tensor.Value<Element, Rank, Tensor.Layout.Order.Row> {
        self.map { (x: Element) -> Element in Element._sinh(x) }
    }
}
