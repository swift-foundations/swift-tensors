...
{
    /// Element-wise cosine (`cos xᵢ`).
    ///
    /// Routes per-element through `Element._cos(_:)` from
    /// `Numeric.Transcendental`, which binds the libm `cos`/`cosf` family
    /// at the L1/L2 boundary per [PLAT-ARCH-008j].
    @inlinable
    public func cos() -> Tensor.Value<Element, Rank, Tensor.Layout.Order.Row> {
        self.map { (x: Element) -> Element in Element._cos(x) }
    }
}
